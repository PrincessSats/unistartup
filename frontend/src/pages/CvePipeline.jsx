...
function ChainStep({ label, value, isFlag, isCiphertext, isFirst, isLast }) {
  const [revealed, setRevealed] = useState(isFirst);
...
function CveSearchWidget({ selectedCve, onSelect }) {
  ...
  async function loadRecent() {
    setSearching(true);
    try {
      const data = await cveAPI.search('', 20);
      setResults(data);
    } catch {
      setResults([]);
    } finally {
  ...
  function handleQueryChange(e) {
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true);
      try {
        const data = await cveAPI.search(val.trim());
        setResults(data);
      } catch {
        setResults([]);
      } finally {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      {selectedCve ? (
        <div className="rounded-[12px] bg-[#9B6BFF]/10 border border-[#9B6BFF]/30 p-3">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[12px] font-mono font-semibold text-[#9B6BFF]">{selectedCve.cve_id}</p>
              {selectedCve.ru_title && (
                <p className="text-[11px] text-white/60 mt-0.5 truncate">{selectedCve.ru_title}</p>
              )}
              {selectedCve.ru_summary && (
                <p className="text-[10px] text-white/35 mt-1 line-clamp-2">{selectedCve.ru_summary}</p>
              )}
            </div>
            <button
              type="button"
              onClick={handleClear}
              className="text-white/30 hover:text-white/70 text-[12px] shrink-0 transition-colors"
            >
              ✕
            </button>
          </div>
          ...
          {open && (
            <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-[#13121A] border border-white/[0.1] rounded-[12px] overflow-hidden shadow-xl max-h-64 overflow-y-auto">
              ...
              {!searching && results.map(cve => (
                ...
                >
                  <p className="text-[11px] font-mono text-[#9B6BFF]">{cve.cve_id || `ID:${cve.id}`}</p>
                  {cve.ru_title && (
                    <p className="text-[10px] text-white/50 mt-0.5 truncate">{cve.ru_title}</p>
                  )}
                </button>
              ))}
            </div>
  ...
  async function handleGenerate() {
    ...
    try {
      const data = await pipelineAPI.startGeneration({
        cve_id: selectedCve.cve_id,
        difficulty,
        num_variants: numVariants,
        ...(taskType !== 'auto' && { task_type: taskType }),
      });
      setActiveBatchId(data.batch_id);
      setBatchStatus({ status: 'pending', current_stage: 'pending', variants: [], difficulty, num_variants: numVariants });
      loadBatches();
    } catch (err) {
